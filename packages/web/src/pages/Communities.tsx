...
export default function Communities() {
  ...
  const [communityBooks, setCommunityBooks] = useState<Record<string, Book[]>>({});

  useEffect(() => {
    api.get<Community[]>('/communities').then(setJoined).catch(() => {});
  }, []);

  async function toggleBooks(communityId: string) {
    if (expanded === communityId) { setExpanded(null); return; }
          ...
          {joined.map(c => (
            <li key={c.id} className={s.card}>
              <div className={s.cardHeader}>
                ...
                <button className={s.expandBtn} onClick={() => toggleBooks(c.id)}>
                  {expanded === c.id ? 'Hide books ▲' : 'View books ▼'}
                </button>
              </div>
              {expanded === c.id && (
                <div className={s.bookList}>
                  {!communityBooks[c.id] ? (
                    <p className={s.bookEmpty}>Loading…</p>
                  ) : communityBooks[c.id]!.length === 0 ? (
                    <p className={s.bookEmpty}>No available books in this community.</p>
                  ) : (
                    communityBooks[c.id]!.map(b => (
                      <div key={b.id} className={s.bookRow}>
                        <span className={s.bookTitle}>{b.title}</span>
                        <span className={s.bookSub}>{b.author ?? 'Unknown author'}</span>
                      </div>
                    ))
                  )}
                </div>
